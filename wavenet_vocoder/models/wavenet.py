...
from .modules import (Conv1D1x1, ConvTranspose2D, ConvTranspose1D, ResizeConvolution,
                      SubPixelConvolution, DiscretizedMixtureLogisticLoss,
                      GaussianMaximumLikelihoodEstimation, LeakyReluActivation,
                      MaskedCrossEntropyLoss, ReluActivation, ResidualConv1DGLU, WeightNorm,
                      Embedding)
...
def _expand_global_features(batch_size, time_length, global_features, data_format='BCT'):
    ...
    if not (data_format in accepted_formats):
        raise ValueError(
            '{} is an unknow data format, accepted formats are "BCT" and "BTC"'.format(data_format))
...
class WaveNet():
                ...
                with tf.variable_scope('local_conditioning_upsampling_{}'.format(i + 1)):
                    if hparams.upsample_type == '2D':
                        convt = ConvTranspose2D(1, (hparams.freq_axis_kernel_size, 2 * s),
                                                ...
                                                NN_init=hparams.NN_init,
                                                NN_scaler=hparams.NN_scaler,
    ...
    def initialize(self, y, c, g, input_lengths, x=None, synthesis_length=None, test_inputs=None,
                   split_infos=None):
        ...
        with tf.device(split_device):
            ...
            tower_input_lengths = tf.split(input_lengths, num_or_size_splits=hp.wavenet_num_gpus,
                                           axis=0) if input_lengths is not None else [
                                                                                         input_lengths] * hp.wavenet_num_gpus

            tower_y = tf.split(y, num_or_size_splits=hp.wavenet_num_gpus,
                               axis=0) if y is not None else [y] * hp.wavenet_num_gpus
            tower_x = tf.split(x, num_or_size_splits=hp.wavenet_num_gpus,
                               axis=0) if x is not None else [x] * hp.wavenet_num_gpus
            tower_c = tf.split(c, num_or_size_splits=hp.wavenet_num_gpus,
                               axis=0) if self.local_conditioning_enabled() else [
                                                                                     None] * hp.wavenet_num_gpus
            tower_g = tf.split(g, num_or_size_splits=hp.wavenet_num_gpus,
                               axis=0) if self.global_conditioning_enabled() else [
                                                                                      None] * hp.wavenet_num_gpus
            tower_test_inputs = tf.split(test_inputs, num_or_size_splits=hp.wavenet_num_gpus,
                                         axis=0) if test_inputs is not None else [
                                                                                     test_inputs] * hp.wavenet_num_gpus
            ...
            with tf.device(tf.train.replica_device_setter(ps_tasks=1, ps_device='/cpu:0',
                ...
                with tf.variable_scope('inference') as scope:
                    ...
                    if self.is_training:
                        ...
                        self.tower_mask.append(self.get_mask(tower_input_lengths[i],
                                                             maxlen=tf.shape(tower_x[i])[
                                                                 -1]))  # To be used in loss computation
                        # [batch_size, channels, time_length]
                        y_hat_train = self.step(tower_x[i], tower_c[i], tower_g[i],
                                                softmax=False)  # softmax is automatically computed inside softmax_cross_entropy if needed
                        ...
                        with tf.control_dependencies(
                                [tf.assert_equal(tf.shape(tower_y[i]), shape_control)]):
                        ...
                        if is_mulaw_quantize(hparams.input_type):
                            ...
                            y_hat_log = util.inv_mulaw_quantize(y_hat_log,
                    ...
                    # evaluating
                        ...
                        test_inputs = tf.reshape(y_target, [1, -1,
                                                            1]) if not hparams.wavenet_natural_eval else None
                        ...
                        if is_mulaw_quantize(hparams.input_type):
                            ...
                            initial_input = tf.tile(
                                tf.reshape(initial_input, [1, 1, hparams.quantize_channels]),
                                [batch_size, 1, 1])
                        else:
                            initial_input = tf.ones([batch_size, 1, 1], tf.float32) * initial_value

                        # Fast eval
                        y_hat = self.incremental(initial_input, c=tower_c[i], g=tower_g[i],
                                                 time_length=length, test_inputs=test_inputs,
                                                 softmax=False, quantize=True,
                        ...
                        self.tower_eval_upsampled_local_features.append(
                            self.upsampled_local_features[idx])
                    ...
                    # synthesizing
                        ...
                        if c is None:
                            ...
                            message = (
                            'Expected 3 dimension shape [batch_size(1), time_length, {}] for local condition features but found {}'.format(
                                hparams.cin_channels, tower_c[i].shape))
                            with tf.control_dependencies(
                                    [tf.assert_equal(tf.rank(tower_c[i]), 3, message=message)]):
                                tower_c[i] = tf.identity(tower_c[i],
                                                         name='synthesis_assert_c_rank_op')
                        ...
                        y_hat = self.incremental(initial_input, c=tower_c[i], g=tower_g[i],
                                                 ...
                                                 test_inputs=tower_test_inputs[i],
                                                 softmax=False, quantize=True,
                                                 log_scale_min=hparams.log_scale_min,
                                                 log_scale_min_gauss=hparams.log_scale_min_gauss)
                        ...
                        self.tower_y_hat.append(y_hat)
                        self.tower_synth_upsampled_local_features.append(
                            self.upsampled_local_features)
        ...
        log('  Receptive Field:           ({} samples / {:.1f} ms)'.format(self.receptive_field,
                                                                           self.receptive_field / hparams.sample_rate * 1000.))

        # 1_000_000 is causing syntax problems for some people?! Python please :)
        log('  WaveNet Parameters:        {:.3f} Million.'.format(
            np.sum([np.prod(v.get_shape().as_list()) for v in self.variables]) / 1000000))
                ...
                with tf.variable_scope('loss') as scope:
                    if self.is_training:
                        if is_mulaw_quantize(self._hparams.input_type):
                            tower_loss = MaskedCrossEntropyLoss(self.tower_y_hat_q[i][:, :-1, :],
                                                                self.tower_y[i][:, 1:],
                                                                mask=self.tower_mask[i])
                        else:
                            if self._hparams.out_channels == 2:
                                tower_loss = GaussianMaximumLikelihoodEstimation(
                                    self.tower_y_hat_train[i][:, :, :-1], self.tower_y[i][:, 1:, :],
                                    hparams=self._hparams, mask=self.tower_mask[i])
                            else:
                                tower_loss = DiscretizedMixtureLogisticLoss(
                                    self.tower_y_hat_train[i][:, :, :-1], self.tower_y[i][:, 1:, :],
                                    hparams=self._hparams, mask=self.tower_mask[i])

                    elif self.is_evaluating:
                        if is_mulaw_quantize(self._hparams.input_type):
                            tower_loss = MaskedCrossEntropyLoss(self.tower_y_hat_eval[i],
                                                                self.tower_y_eval[i],
                                                                lengths=[self.tower_eval_length[i]])
                        else:
                            if self._hparams.out_channels == 2:
                                tower_loss = GaussianMaximumLikelihoodEstimation(
                                    self.tower_y_hat_eval[i], self.tower_y_eval[i],
                        ...
                        raise RuntimeError(
                            'Model not in train/eval mode but computing loss: Where did this go wrong?')
    ...
    def add_optimizer(self, global_step):
        ...
        with tf.device(grad_device):
            ...
            if hp.wavenet_clip_gradients:
                ...
                for g in avg_grads:
                    if g is not None:
                        ...
                        clipped_g = tf.clip_by_value(clipped_g, -hp.wavenet_gradient_max_value,
                                                     hp.wavenet_gradient_max_value)
            ...
            with tf.control_dependencies([adam_optimize]):
                ...
                assert set(self.variables) == set(
                    variables)  # Verify all trainable variables are being averaged
    ...
    def incremental(self, initial_input, c=None, g=None,
        ...
        if test_inputs is not None:
            if self.scalar_input:
                ...
                test_inputs = tf.one_hot(indices=test_inputs, depth=self._hparams.quantize_channels,
                                         dtype=tf.float32)
        ...
        initial_queues = [tf.zeros((batch_size, res_conv.layer.kw + (res_conv.layer.kw - 1) * (
                    res_conv.layer.dilation_rate[0] - 1), self._hparams.residual_channels),
                                   name='convolution_queue_{}'.format(i + 1)) for i, res_conv in
                          enumerate(self.residual_layers)]
        ...
        def body(time, outputs_ta, current_input, loss_outputs_ta, queues):
            ...
            loss_outputs_ta = loss_outputs_ta.write(time, tf.squeeze(x, [
                1]))  # squeeze time_length dimension (=1)
            ...
            if self.scalar_input:
                ...
                if quantize:
                    # [batch_size, 1]
                    sample = tf.multinomial(x,
                                            1)  # Pick a sample using x as probability (one for each batch)
