...
class CausalConv1D(tf.keras.layers.Wrapper):
    ...
    def build(self, input_shape):
        ...
        in_channels = input_shape[1] if self.layer.data_format == 'channels_first' else input_shape[
            -1]
        ...
        with tf.variable_scope(self.scope) as scope:
            if incremental:
                ...
                if self.kw > 1:
                    ...
                    # append next input
                    convolution_queue = tf.concat(
                        [convolution_queue, tf.expand_dims(inputs[:, -1, :], axis=1)], axis=1)
            ...
            with tf.control_dependencies(
                    [tf.assert_equal(tf.shape(outputs)[time_dim], tf.shape(inputs)[time_dim])]):
...
class Conv1D1x1(CausalConv1D):
    ...
    def call(self, inputs, incremental=False, convolution_queue=None):
        with tf.variable_scope(self.scope) as scope:
            ...
            return super(Conv1D1x1, self).call(inputs, incremental=incremental,
                                               convolution_queue=convolution_queue)
...
class ResidualConv1DGLU(tf.keras.layers.Wrapper):
    ...
    def __init__(self, residual_channels, gate_channels, kernel_size,
                 ...
                 weight_normalization=True, init=False, init_scale=1., residual_legacy=True,
...
class SubPixelConvolution(tf.layers.Conv2D):
    ...
    def __init__(self, filters, kernel_size, padding, strides, NN_init, NN_scaler, name=None,
                 **kwargs):
        ...
        init_kernel = tf.constant_initializer(self._init_kernel(kernel_size, strides, conv_filters),
                                              dtype=tf.float32) if NN_init else None
    ...
    def PS(self, inputs):
        ...
        outputs = tf.concat([self._phase_shift(x, batch_size, H, W, r1, r2) for x in Xc],
                            3)  # [batch_size, r1 * H, r2 * W, out_c]
    ...
    def _init_kernel(self, kernel_size, strides, filters):
        ...
        i = kernel_size[0] // 2
        j = [kernel_size[1] // 2 - 1, kernel_size[1] // 2] if kernel_size[1] % 2 == 0 else [
            kernel_size[1] // 2]
        for j_i in j:
...
class ResizeConvolution(tf.layers.Conv2D):
    def __init__(self, filters, kernel_size, padding, strides, NN_init, NN_scaler, name=None,
        ...
        # Create initial kernel
        self.NN_scaler = NN_scaler
        init_kernel = tf.constant_initializer(self._init_kernel(kernel_size, strides),
                                              dtype=tf.float32) if NN_init else None
    ...
    def call(self, inputs):
        with tf.variable_scope(self.scope) as scope:
            ...
            resized = tf.image.resize_images(
                inputs,
                size=[inputs.shape[1] * self.resize_strides[0],
                      tf.shape(inputs)[2] * self.resize_strides[1]],
...
class ConvTranspose1D(tf.layers.Conv2DTranspose):
    def __init__(self, filters, kernel_size, padding, strides, NN_init, NN_scaler, name=None,
        ...
        kernel_size = (
                          1,) + kernel_size  # (ks, ) -> (1, ks). Inputs supposed [batch_size, channels, freq, time_steps].
        strides = (1,) + strides  # (s, ) -> (1, s).

        # Create initial kernel
        self.NN_scaler = NN_scaler
        init_kernel = tf.constant_initializer(self._init_kernel(kernel_size, strides, filters),
                                              dtype=tf.float32) if NN_init else None
    ...
    def _init_kernel(self, kernel_size, strides, filters):
        ...
        init_kernel = np_utils.to_categorical(init_kernel, num_classes=len(init_kernel)).reshape(1,
                                                                                                 1,
                                                                                                 -1,
                                                                                                 filters).astype(
            np.float32)
...
def DiscretizedMixtureLogisticLoss(outputs, targets, hparams, lengths=None, mask=None,
                                   max_len=None):
    if lengths is None and mask is None:
        raise RuntimeError('Please provide either lengths or mask')
...
def GaussianMaximumLikelihoodEstimation(outputs, targets, hparams, lengths=None, mask=None,
                                        max_len=None):
    if lengths is None and mask is None:
        raise RuntimeError('Please provide either lengths or mask')
