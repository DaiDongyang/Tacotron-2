...
def gaussian_maximum_likelihood_estimation_loss(y_hat, y, log_scale_min_gauss, num_classes,
                                                use_cdf=True, reduce=True):
    '''compute the gaussian MLE loss'''
    with tf.control_dependencies(
            [tf.assert_equal(tf.shape(y_hat)[1], 2), tf.assert_equal(tf.rank(y_hat), 3)]):
    ...
    if use_cdf:
        ...
        # This is the log(PDF)
        log_prob = -0.5 * (np.log(2. * np.pi) + 2. * log_scale + tf.square(y - mean) * tf.exp(
            -2. * log_scale))
