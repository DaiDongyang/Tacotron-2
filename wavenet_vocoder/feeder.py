...
class Feeder:
    ...
    def __init__(self, coordinator, metadata_filename, base_dir, hparams):
        ...
        train_indices, test_indices = train_test_split(indices,
                                                       test_size=test_size,
                                                       random_state=hparams.wavenet_data_random_state)
        ...
        with tf.device('/cpu:0'):
            ...
            if is_scalar_input(hparams.input_type):
                input_placeholder = tf.placeholder(tf.float32, shape=(None, 1, None),
                                                   name='audio_inputs')
                target_placeholder = tf.placeholder(tf.float32, shape=(None, None, 1),
                                                    name='audio_targets')
                target_type = tf.float32
            else:
                input_placeholder = tf.placeholder(tf.float32,
                                                   shape=(None, hparams.quantize_channels, None),
                                                   name='audio_inputs')
                target_placeholder = tf.placeholder(tf.int32, shape=(None, None, 1),
                                                    name='audio_targets')
            ...
            if self.local_condition:
                self._placeholders.append(
                    tf.placeholder(tf.float32, shape=(None, hparams.num_mels, None),
                                   name='local_condition_features'))
                queue_types.append(tf.float32)
            if self.global_condition:
                self._placeholders.append(
                    tf.placeholder(tf.int32, shape=(None, 1), name='global_condition_features'))
    ...
    def _get_test_groups(self):
        ...
        if self.global_condition:
            ...
            if global_condition_features == '<no_g>':
                raise RuntimeError(
                    'Please redo the wavenet preprocessing (or GTA synthesis) to assign global condition features!')
        else:
    ...
    def make_test_batches(self):
        ...
        log('\nGenerated {} test batches of size {} in {:.3f} sec'.format(len(batches), n,
                                                                          time.time() - start))
        return batches
    ...
    def _enqueue_next_train_group(self):
        while not self._coord.should_stop():
            ...
            log('\nGenerated {} train batches of size {} in {:.3f} sec'.format(len(batches), n,
                                                                               time.time() - start))
    ...
    def _get_next_example(self):
        ...
        if self._hparams.train_with_GTA:
            ...
            if 'linear' in mel_file:
                raise RuntimeError(
                    'Linear spectrogram files selected instead of GTA mels, did you specify the wrong metadata?')
        else:
            mel_file = meta[1]
        audio_file = meta[0]
    ...
    def _prepare_batch(self, batches):
        ...
        local_condition_features = self._prepare_local_conditions(self.local_condition,
                                                                  [x[1] for x in batches])
        global_condition_features = self._prepare_global_conditions(self.global_condition,
    ...
    def _prepare_inputs(self, inputs, maxlen):
        if is_mulaw_quantize(self._hparams.input_type):
            ...
            x_batch = np.stack([_pad_inputs(np_utils.to_categorical(
                x, num_classes=self._hparams.quantize_channels), maxlen) for x in inputs]).astype(
                np.float32)
        else:
            # [batch_size, time_steps, 1]
            x_batch = np.stack([_pad_inputs(x.reshape(-1, 1), maxlen) for x in inputs]).astype(
                np.float32)
    ...
    def _prepare_local_conditions(self, local_condition, c_features):
        if local_condition:
            ...
            if self._hparams.clip_for_wavenet:
                c_features = [np.clip(x, T2_output_range[0], T2_output_range[1]) for x in
                              c_features]

            c_batch = np.stack(
                [_pad_inputs(x, maxlen, _pad=T2_output_range[0]) for x in c_features]).astype(
                np.float32)
        ...
        if local_condition:
            ...
            for b in batch:
                ...
                if max_time_steps is not None:
                    max_steps = _ensure_divisible(max_time_steps, audio.get_hop_size(self._hparams),
                                                  True)
                    if len(x) > max_time_steps:
                        ...
                        x = x[time_start: time_start + max_time_frames * audio.get_hop_size(
                            self._hparams)]
