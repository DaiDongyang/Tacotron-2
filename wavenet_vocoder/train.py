...
def add_train_stats(model, hparams):
    with tf.variable_scope('stats') as scope:
        ...
        tf.summary.scalar('max_gradient_norm', tf.reduce_max(
            gradient_norms))  # visualize gradients (in case of explosion)
        return tf.summary.merge_all()


def add_test_stats(summary_writer, step, eval_loss, hparams):
    values = [
        tf.Summary.Value(tag='Wavenet_eval_model/eval_stats/wavenet_eval_loss',
...
def create_shadow_saver(model, global_step=None):
    ...
    shadow_dict = dict(zip(shadow_variables,
                           variables))  # dict(zip(keys, values)) -> {key1: value1, key2: value2, ...}
...
def eval_step(sess, global_step, model, plot_dir, wav_dir, summary_writer, hparams, model_name):
    ...
    y_hat, y_target, loss, input_mel, upsampled_features = sess.run(
        [model.tower_y_hat[0], model.tower_y_target[0],
    ...
    mel_path = os.path.join(plot_dir,
    ...
    # Save Audio
    save_wavenet_wav(y_hat, pred_wav_path, sr=hparams.sample_rate,
                     inv_preemphasize=hparams.preemphasize, k=hparams.preemphasis)
    save_wavenet_wav(y_target, target_wav_path, sr=hparams.sample_rate,
                     inv_preemphasize=hparams.preemphasize, k=hparams.preemphasis)

    # Save figure
    util.waveplot(plot_path, y_hat, y_target, model._hparams,
                  title='{}, {}, step={}, loss={:.5f}'.format(model_name, time_string(),
                                                              global_step, loss))
    ...
    T2_output_range = (
    -hparams.max_abs_value, hparams.max_abs_value) if hparams.symmetric_mels else (
...
def save_log(sess, global_step, model, plot_dir, wav_dir, hparams, model_name):
    ...
    y_hat, y, loss, length, input_mel, upsampled_features = sess.run([model.tower_y_hat_log[0][idx],
                                                                      ...
                                                                      model.loss,
                                                                      model.tower_input_lengths[0][
                                                                          idx],
                                                                      model.tower_c[0][idx],
                                                                      model.tower_upsampled_local_features[
                                                                          0][idx]])
    ...
    save_wavenet_wav(y_hat, pred_wav_path, sr=hparams.sample_rate,
                     inv_preemphasize=hparams.preemphasize, k=hparams.preemphasis)
    save_wavenet_wav(y, target_wav_path, sr=hparams.sample_rate,
                     inv_preemphasize=hparams.preemphasize, k=hparams.preemphasis)
...
def model_train_mode(args, feeder, hparams, global_step, init=False):
    with tf.variable_scope('WaveNet_model', reuse=tf.AUTO_REUSE) as scope:
        ...
        model.initialize(feeder.targets, feeder.local_condition_features,
                         feeder.global_condition_features,
...
def model_test_mode(args, feeder, hparams, global_step):
    with tf.variable_scope('WaveNet_model', reuse=tf.AUTO_REUSE) as scope:
        ...
        model.initialize(feeder.eval_targets, feeder.eval_local_condition_features,
                         feeder.eval_global_condition_features,
        ...
        try:
            ...
            while not coord.should_stop() and step < args.wavenet_train_steps:
                ...
                if step % args.eval_interval == 0:
                    ...
                    eval_step(sess, step, eval_model, eval_plot_dir, eval_wav_dir,
                              summary_writer=summary_writer, hparams=model._hparams,
                              model_name=args.model)
