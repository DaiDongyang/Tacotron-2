...
def run_synthesis(args, checkpoint_path, output_dir, hparams):
    ...
    if args.model == 'Tacotron-2':
        ...
        mel_files = sorted([os.path.join(args.mels_dir, f) for f in os.listdir(args.mels_dir) if
                            f.split('.')[-1] == 'npy'])
    ...
    speaker_ids = None if speaker_ids is None else [
        speaker_ids[i: i + hparams.wavenet_synthesis_batch_size] for i in
        range(0, len(speaker_ids), hparams.wavenet_synthesis_batch_size)]
    ...
    with open(os.path.join(wav_dir, 'map.txt'), 'w') as file:
        for i, mel_batch in enumerate(tqdm(mel_files)):
            ...
            audio_files = synth.synthesize(mel_spectros, speaker_id_batch, basenames, wav_dir,
                                           log_dir)
