...
class Synthesizer:
    def load(self, checkpoint_path, hparams, model_name='WaveNet'):
        ...
        self.local_conditions = tf.placeholder(tf.float32, shape=(None, None, hparams.num_mels),
                                               name='local_condition_features') if local_cond else None
        self.global_conditions = tf.placeholder(tf.int32, shape=(None, 1),
                                                name='global_condition_features') if global_cond else None
        self.synthesis_length = tf.placeholder(tf.int32, shape=(),
                                               name='synthesis_length') if not local_cond else None
        self.targets = tf.placeholder(tf.float32, shape=(1, None, 1),
                                      name='audio_targets') if hparams.wavenet_synth_debug else None  # Debug only with 1 wav
        self.input_lengths = tf.placeholder(tf.int32, shape=(1,),
        ...
        with tf.variable_scope('WaveNet_model') as scope:
            ...
            self.model.initialize(y=None, c=self.local_conditions, g=self.global_conditions,
                                  ...
                                  synthesis_length=self.synthesis_length, test_inputs=self.targets)
    ...
    def synthesize(self, mel_spectrograms, speaker_ids, basenames, out_dir, log_dir):
        ...
        # [-max, max] or [0,max]
        T2_output_range = (-self._hparams.max_abs_value,
        ...
        if self._hparams.clip_for_wavenet:
            mel_spectrograms = [np.clip(x, T2_output_range[0], T2_output_range[1]) for x in
                                mel_spectrograms]

        c_batch = np.stack(
            [_pad_inputs(x, maxlen, _pad=T2_output_range[0]) for x in mel_spectrograms]).astype(
            np.float32)
        ...
        g = None if speaker_ids is None else np.asarray(speaker_ids, dtype=np.int32).reshape(
            len(c_batch), 1)
        feed_dict = {}
        ...
        if self.synth_debug:
            ...
            test_wavs = np.stack([_pad_inputs(x, max_test_len) for x in test_wavs]).astype(
                np.float32)
        ...
        generated_wavs, upsampled_features = self.session.run(
            [self.model.tower_y_hat, self.model.tower_synth_upsampled_local_features],
            feed_dict=feed_dict)
        ...
        generated_wavs = [generated_wav[:length] for generated_wav, length in
                          zip(generated_wavs, audio_lengths)]
        upsampled_features = [upsampled_feature[:, :length] for upsampled_feature, length in
                              zip(upsampled_features, audio_lengths)]
        ...
        for i, (generated_wav, input_mel, upsampled_feature) in enumerate(
            ...
            save_wavenet_wav(generated_wav, audio_filename, sr=hparams.sample_rate,
                             inv_preemphasize=hparams.preemphasize, k=hparams.preemphasis)
            ...
            util.plot_spectrogram(generated_mel, os.path.join(log_dir,
                                                              'wavenet-mel-spectrogram-{}.png'.format(
                                                                  basenames[i])),
                                  title='Local Condition vs Reconstructed Audio Mel-Spectrogram analysis',
                                  target_spectrogram=input_mel)
            # Save upsampled features to visualize checkerboard artifacts.
            util.plot_spectrogram(upsampled_feature.T, os.path.join(log_dir,
                                                                    'wavenet-upsampled_features-{}.png'.format(
                                                                        basenames[i])),
                                  title='Upmsampled Local Condition features', auto_aspect=True)

            # Save waveplot to disk
            if log_dir is not None:
                plot_filename = os.path.join(log_dir,
                                             'wavenet-waveplot-{}.png'.format(basenames[i]))
