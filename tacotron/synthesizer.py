...
class Synthesizer:
    ...
    def synthesize(self, texts, basenames, out_dir, log_dir, mel_filenames):
        ...
        for i in range(self._hparams.tacotron_num_gpus):
            ...
            input_seqs = np.concatenate((input_seqs, device_input),
        ...
        if self.gta:
            ...
            for i in range(self._hparams.tacotron_num_gpus):
                ...
                device_target, max_target_len = self._prepare_targets(device_target,
        ...
        if self.gta or not hparams.predict_linear:
            ...
            # Take off the batch wise padding
            mels = [mel[:target_length, :] for mel, target_length in zip(mels, target_lengths)]
            linears = [linear[:target_length, :] for linear, target_length in
                       zip(linears, target_lengths)]
        ...
        for i, mel in enumerate(mels):
            ...
            if log_dir is not None:
                ...
                audio.save_wav(wav,
                               os.path.join(log_dir, 'wavs/wav-{}-mel.wav'.format(basenames[i])),
                               sr=hparams.sample_rate)

                # save alignments
                plot.plot_alignment(alignments[i], os.path.join(log_dir,
                ...
                plot.plot_spectrogram(mel, os.path.join(log_dir,
                ...
                if hparams.predict_linear:
                    ...
                    audio.save_wav(wav, os.path.join(log_dir,
                                                     'wavs/wav-{}-linear.wav'.format(basenames[i])),
                                   sr=hparams.sample_rate)

                    # save linear spectrogram plot
                    plot.plot_spectrogram(linears[i], os.path.join(log_dir,
    ...
    def _get_output_lengths(self, stop_tokens):
        ...
        output_lengths = [row.index(1) if 1 in row else len(row) for row in
                          np.round(stop_tokens).tolist()]
