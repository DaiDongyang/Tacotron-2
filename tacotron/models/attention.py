  ...
  577–585.
    ...
    def __call__(self, query, state, prev_max_attentions):
        ...
        with variable_scope.variable_scope(None, "Location_Sensitive_Attention", [query]):
            ...
            energy = _location_sensitive_score(processed_query, processed_location_features,
                                               self.keys)
        ...
        if self.synthesis_constraint:
            ...
            if self.constraint_type == 'monotonic':
                ...
                reverse_masks = tf.sequence_mask(Tx - self.attention_win_size - prev_max_attentions,
                                                 Tx)[:, ::-1]
            else:
                assert self.constraint_type == 'window'
                key_masks = tf.sequence_mask(prev_max_attentions - (
                            self.attention_win_size // 2 + (self.attention_win_size % 2 != 0)), Tx)
                reverse_masks = tf.sequence_mask(
                    Tx - (self.attention_win_size // 2) - prev_max_attentions, Tx)[:, ::-1]
