...
class TacoTestHelper(Helper):
    ...
    def initialize(self, name=None):
        return (
        tf.tile([False], [self._batch_size]), _go_frames(self._batch_size, self._output_dim))
        ...
        if self.gta:
            ...
            if self._hparams.tacotron_teacher_forcing_mode == 'scheduled':
                self._ratio = _teacher_forcing_ratio_decay(
                    self._hparams.tacotron_teacher_forcing_init_ratio,
...
def _teacher_forcing_ratio_decay(init_tfr, global_step, hparams):
    ...
    if hparams.tacotron_teacher_forcing_final_ratio is not None:
        alpha = float(
            hparams.tacotron_teacher_forcing_final_ratio / hparams.tacotron_teacher_forcing_init_ratio)
