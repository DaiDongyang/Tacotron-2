...
class Feeder:
    ...
    def __init__(self, coordinator, metadata_filename, hparams):
        ...
        with tf.device('/cpu:0'):
            ...
            eval_queue = tf.FIFOQueue(1, [tf.int32, tf.int32, tf.float32, tf.float32, tf.float32,
    ...
    def _prepare_batch(self, batches, outputs_per_step):
        ...
        for i in range(self._hparams.tacotron_num_gpus):
            ...
            inputs = np.concatenate((inputs, input_cur_device),
                                    axis=1) if inputs is not None else input_cur_device
            mel_target_cur_device, mel_target_max_len = self._prepare_targets([x[1] for x in batch],
                                                                              outputs_per_step)
            ...
            token_target_cur_device, token_target_max_len = self._prepare_token_targets(
            ...
            linear_targets_cur_device, linear_target_max_len = self._prepare_targets(
                [x[3] for x in batch], outputs_per_step)
            ...
            split_infos.append(
                [input_max_len, mel_target_max_len, token_target_max_len, linear_target_max_len])
